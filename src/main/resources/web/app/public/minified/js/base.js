...
+function($) {
    ...
    function createInjector(modulesToLoad, strictDi) {
        ...
        function createInternalInjector(cache, factory) {
            ...
            function isClass(func) {
                return 11 >= msie ? !1 : "function" == typeof func && /^(?:class\s|constructor\()/.test(stringifyFn(func));
            }
            ...
            function flushOnChangesQueue() {
                try {
                    ...
                    $rootScope.$apply(function() {
                        for (var i = 0, ii = onChangesQueue.length; ii > i; ++i) onChangesQueue[i]();
            ...
            function applyDirectivesToNode(directives, compileNode, templateAttrs, transcludeFn, jqCollection, originalReplaceDirective, preLinkFns, postLinkFns, previousCompileContext) {
                ...
                function nodeLinkFn(childLinkFn, scope, linkNode, $rootElement, boundTranscludeFn) {
                    ...
                    for (forEach(controllerDirectives, function(controllerDirective, name) {
                        ...
                        isFunction(controllerInstance.$onChanges) && controllerInstance.$onChanges(controller.bindingInfo.initialChanges), 
                        isFunction(controllerInstance.$onInit) && controllerInstance.$onInit(), isFunction(controllerInstance.$onDestroy) && controllerScope.$on("$destroy", function() {
    ...
    function numberInputType(scope, element, attr, ctrl, $sniffer, $browser) {
        ...
        ctrl.$$parserName = "number", ctrl.$parsers.push(function(value) {
            ...
            ctrl.$validators.min = function(value) {
                ...
                isDefined(val) && !isNumber(val) && (val = parseFloat(val, 10)), minVal = isNumber(val) && !isNaN(val) ? val : void 0, 
                ctrl.$validate();
            });
        }
        if (isDefined(attr.max) || attr.ngMax) {
            ...
            ctrl.$validators.max = function(value) {
                ...
                isDefined(val) && !isNumber(val) && (val = parseFloat(val, 10)), maxVal = isNumber(val) && !isNaN(val) ? val : void 0, 
                ctrl.$validate();
            });
        }
    }
    ...
    var ngIfDirective = [ "$animate", "$compile", function($animate, $compile) {
        ...
        function ngOptionsPostLink(scope, selectElement, attr, ctrls) {
            ...
            function updateOptions() {
                ...
                if (providedEmptyOption && selectElement.prepend(emptyOption), options.items.forEach(function(option) {
                    ...
                    listFragment.appendChild(groupElement), groupElement.label = option.group, groupElementMap[option.group] = groupElement), 
        ...
        link: function($scope, $element, $attrs, controller, $transclude) {
            ...
            !$transclude) throw ngTranscludeMinErr("orphan", "Illegal use of ngTransclude directive in the template! No parent directive that requires a transclusion found. Element: {0}", startingTag($element));
...
function(window, angular) {
    ...
    function htmlParser(html, handler) {
        ...
        for (var node = inertBodyElement.firstChild; node; ) {
            ...
            nextNode = node.nextSibling, !nextNode)) for (;null == nextNode && (node = node.parentNode, 
            node !== inertBodyElement); ) nextNode = node.nextSibling, 1 == node.nodeType && handler.end(node.nodeName.toLowerCase());
            node = nextNode;
        }
    ...
    function htmlSanitizeWriter(buf, uriValidator) {
        ...
        return {
            start: function(tag, attrs) {
                tag = angular.lowercase(tag), !ignoreCurrentElement && blockedElements[tag] && (ignoreCurrentElement = tag), 
                ignoreCurrentElement || validElements[tag] !== !0 || (out("<"), out(tag), angular.forEach(attrs, function(value, key) {
                ...
                tag = angular.lowercase(tag), ignoreCurrentElement || validElements[tag] !== !0 || voidElements[tag] === !0 || (out("</"), 
    ...
    }(window), angular.module("ngSanitize", []).provider("$sanitize", $SanitizeProvider), 
    angular.module("ngSanitize").filter("linky", [ "$sanitize", function($sanitize) {
        var LINKY_URL_REGEXP = /((ftp|https?):\/\/|(www\.)|(mailto:)?[A-Za-z0-9._%+-]+@)\S*[^\s.;,(){}<>"\u201d\u2019]/i, MAILTO_REGEXP = /^mailto:/i, linkyMinErr = angular.$$minErr("linky"), isString = angular.isString;
        return function(text, target, attributes) {
            ...
            function addLink(url, text) {
                var key;
                if (html.push("<a "), angular.isFunction(attributes) && (attributes = attributes(url)), 
                angular.isObject(attributes)) for (key in attributes) html.push(key + '="' + attributes[key] + '" '); else attributes = {};
                !angular.isDefined(target) || "target" in attributes || html.push('target="', target, '" '), 
...
function(window, angular, undefined) {
    ...
    "use strict";
    angular.module("ui.router.title", [ "ui.router" ]).run([ "$rootScope", "$timeout", "$state", function($rootScope, $timeout, $state) {
        ...
        $rootScope.$on("$stateChangeSuccess", function() {
            ...
            for (var state = $state.$current; state; ) state.resolve && state.resolve.$title && $rootScope.$breadcrumbs.unshift({
...
}(window), angular.Module, angular.module("ui.bootstrap", [ "ui.bootstrap.collapse", "ui.bootstrap.accordion", "ui.bootstrap.alert", "ui.bootstrap.buttons", "ui.bootstrap.carousel", "ui.bootstrap.dateparser", "ui.bootstrap.isClass", "ui.bootstrap.datepicker", "ui.bootstrap.position", "ui.bootstrap.datepickerPopup", "ui.bootstrap.debounce", "ui.bootstrap.dropdown", "ui.bootstrap.stackedMap", "ui.bootstrap.modal", "ui.bootstrap.paging", "ui.bootstrap.pager", "ui.bootstrap.pagination", "ui.bootstrap.tooltip", "ui.bootstrap.popover", "ui.bootstrap.progressbar", "ui.bootstrap.rating", "ui.bootstrap.tabs", "ui.bootstrap.timepicker", "ui.bootstrap.typeahead" ]), 
angular.module("ui.bootstrap.collapse", []).directive("uibCollapse", [ "$animate", "$q", "$parse", "$injector", function($animate, $q, $parse, $injector) {
    ...
    return {
        link: function(scope, element, attrs) {
            ...
            function collapseDone() {
                element.css({
                    height: "0"
                }), element.removeClass("collapsing").addClass("collapse"), collapsedExpr(scope);
            }
            ...
            scope.$eval(attrs.uibCollapse) || element.addClass("in").addClass("collapse").attr("aria-expanded", !0).attr("aria-hidden", !1).css({
                height: "auto"
    ...
    this.init = function() {
        ...
        this.formatters[format] || (this.formatters[format] = createParser(format, "formatter"));
        var parser = this.formatters[format], map = parser.map, _format = format;
        return map.reduce(function(str, mapper, i) {
            var match = _format.match(new RegExp("(.*)" + mapper.key));
            match && angular.isString(match[1]) && (str += match[1], _format = _format.replace(match[1] + mapper.key, ""));
            var endStr = i === map.length - 1 ? _format : "";
            return mapper.apply ? str + mapper.apply.call(null, date) + endStr : str + endStr;
        }, "");
    ...
    !!$attrs.datepickerOptions;
    $scope.datepickerOptions || ($scope.datepickerOptions = {}), this.modes = [ "day", "month", "year" ], 
    ...
    $scope.disabled = angular.isDefined($attrs.disabled) || !1, angular.isDefined($attrs.ngDisabled) && watchListeners.push($scope.$parent.$watch($attrs.ngDisabled, function(disabled) {
        ...
        ngModelCtrl = ngModelCtrl_, ngModelOptions = ngModelCtrl_.$options || datepickerConfig.ngModelOptions, 
    ...
    function parseDate(viewValue) {
        ...
        if (angular.isString(viewValue)) {
            ...
            if (!isNaN(date)) return dateParser.toTimezone(date, timezone);
        }
    ...
    function validator(modelValue, viewValue) {
        ...
        value ? angular.isDate(value) && !isNaN(value) ? !0 : angular.isString(value) ? !isNaN(parseDateString(viewValue)) : !1 : !0) : !0;
    }
    ...
    var dateFormat, closeOnDateSelection, appendToBody, onOpenFocus, datepickerPopupTemplateUrl, datepickerTemplateUrl, popupEl, datepickerEl, scrollParentEl, ngModel, ngModelOptions, $popup, altInputFormats, timezone, isHtml5DateInput = !1, watchListeners = [];
    this.init = function(_ngModel_) {
        if (ngModel = _ngModel_, ngModelOptions = _ngModel_.$options, closeOnDateSelection = angular.isDefined($attrs.closeOnDateSelection) ? $scope.$parent.$eval($attrs.closeOnDateSelection) : datepickerPopupConfig.closeOnDateSelection, 
        ...
        ngModelOptions ? (timezone = ngModelOptions.timezone, $scope.ngModelOptions = angular.copy(ngModelOptions), 
        $scope.ngModelOptions.timezone = null, $scope.ngModelOptions.updateOnDefault === !0 && ($scope.ngModelOptions.updateOn = $scope.ngModelOptions.updateOn ? $scope.ngModelOptions.updateOn + " default" : "default"), 
        popupEl.attr("ng-model-options", "ngModelOptions")) : timezone = null, popupEl.attr({
            "ng-model": "date",
        ...
        isHtml5DateInput ? ngModel.$formatters.push(function(value) {
            return $scope.date = dateParser.fromTimezone(value, timezone), value;
        ...
        ngModel.$formatters.push(function(value) {
            ...
            $scope.date = dateParser.fromTimezone(value, timezone), dateParser.filter($scope.date, dateFormat));
        ...
        "today" === date && (date = dateParser.fromTimezone(new Date(), timezone));
        var dates = {};
        return angular.forEach([ "minDate", "maxDate" ], function(key) {
            $scope.datepickerOptions[key] ? angular.isDate($scope.datepickerOptions[key]) ? dates[key] = dateParser.fromTimezone(new Date($scope.datepickerOptions[key]), timezone) : ($datepickerPopupLiteralWarning && $log.warn("Literal date support has been deprecated, please switch to date object usage"), 
    ...
    this.open = function(dropdownScope, element) {
        openScope || ($document.on("click", closeDropdown), element.on("keydown", keybindFilter)), 
    ...
    var closeDropdown = function(evt) {
        ...
        27 === evt.which ? (evt.stopPropagation(), openScope.focusToggleElement(), closeDropdown()) : openScope.isKeynavEnabled() && -1 !== [ 38, 40 ].indexOf(evt.which) && openScope.isOpen && (evt.preventDefault(), 
    ...
    $element.addClass("dropdown"), this.init = function() {
        ...
        if (appendTo && self.dropdownMenu) {
            var css, rightalign, scrollbarWidth, pos = $position.positionElements($element, self.dropdownMenu, "bottom-left", !0);
            if (css = {
                top: pos.top + "px",
                display: isOpen ? "block" : "none"
            }, rightalign = self.dropdownMenu.hasClass("dropdown-menu-right"), rightalign ? (css.left = "auto", 
            scrollbarWidth = $position.scrollbarWidth(!0), css.right = window.innerWidth - scrollbarWidth - (pos.left + $element.prop("offsetWidth")) + "px") : (css.left = pos.left + "px", 
        ...
        }), scope.focusToggleElement(), uibDropdownService.open(scope, $element); else {
            if (self.dropdownMenuTemplateUrl) {
                templateScope && templateScope.$destroy();
                var newEl = angular.element('<ul class="dropdown-menu"></ul>');
                self.dropdownMenu.replaceWith(newEl), self.dropdownMenu = newEl;
            }
            uibDropdownService.close(scope, $element), self.selectedOption = null;
        }
    ...
    function removeAfterAnimate(domEl, scope, done, closedDeferred) {
        function afterAnimating() {
            afterAnimating.done || (afterAnimating.done = !0, $animate.leave(domEl).then(function() {
                domEl.remove(), closedDeferred && closedDeferred.resolve();
            }), scope.$destroy(), done && done());
        }
    ...
    }, topModalIndex = 0, previousTopOpenedModal = null, tabableSelector = "a[href], area[href], input:not([disabled]), button:not([disabled]),select:not([disabled]), textarea:not([disabled]), iframe, object, embed, *[tabindex], *[contenteditable=true]";
    return $rootScope.$watch(backdropIndex, function(newBackdropIndex) {
        ...
        backdropDomEl.attr("backdrop-class", modal.backdropClass), modal.animation && backdropDomEl.attr("modal-animation", "true"), 
        $compile(backdropDomEl)(backdropScope), $animate.enter(backdropDomEl, appendToElement), 
        scrollbarPadding = $uibPosition.scrollbarPadding(appendToElement), scrollbarPadding.heightOverflow && scrollbarPadding.scrollbarWidth && appendToElement.css({
        ...
        appendToElement.addClass(modalBodyClass), $animate.enter($compile(angularDomEl)(modal.scope), appendToElement), 
    ...
    var $modalProvider = {
        ...
        $get: [ "$rootScope", "$q", "$document", "$templateRequest", "$controller", "$uibResolve", "$uibModalStack", function($rootScope, $q, $document, $templateRequest, $controller, $uibResolve, $modalStack) {
            ...
            return $modal.getPromiseChain = function() {
                ...
                modalOptions.appendTo = modalOptions.appendTo || $document.find("body").eq(0), !modalOptions.template && !modalOptions.templateUrl) throw new Error("One of template or templateUrl options is required.");
                var samePromise, templateAndResolvePromise = $q.all([ getTemplatePromise(modalOptions), $uibResolve.resolve(modalOptions.resolve, {}, null, null) ]);
                return samePromise = promiseChain = $q.all([ promiseChain ]).then(resolveWithTemplate, resolveWithTemplate).then(function(tplAndVars) {
                    ...
                    modalOptions.controller && (ctrlLocals.$scope = modalScope, ctrlLocals.$scope.$resolve = {}, 
                    ctrlLocals.$uibModalInstance = modalInstance, angular.forEach(tplAndVars[1], function(value, key) {
                        ctrlLocals[key] = value, ctrlLocals.$scope.$resolve[key] = value;
    ...
    this.options = function(value) {
        ...
        return $document.on("keypress", keypressListener), $rootScope.$on("$destroy", function() {
                ...
                compile: function(tElem, tAttrs) {
                    ...
                    return function(scope, element, attrs, tooltipCtrl) {
                        ...
                        function removeTooltip() {
                            ...
                            tooltip = null), tooltipLinkedScope && (tooltipLinkedScope.$destroy(), tooltipLinkedScope = null);
                        }
                        ...
                        var tooltip, tooltipLinkedScope, transitionTimeout, showTimeout, hideTimeout, positionTimeout, lastPlacement, appendToBody = angular.isDefined(options.appendToBody) ? options.appendToBody : !1, triggers = getTriggers(void 0), hasEnableExp = angular.isDefined(attrs[prefix + "Enable"]), ttScope = scope.$new(!0), repositionScheduled = !1, isOpenParse = angular.isDefined(attrs[prefix + "IsOpen"]) ? $parse(attrs[prefix + "IsOpen"]) : !1, contentParse = options.useContentExp ? $parse(attrs[ttType]) : !1, observers = [], positionTooltip = function() {
                            tooltip && tooltip.html() && (positionTimeout || (positionTimeout = $timeout(function() {
                                ...
                                }), tooltip.hasClass(ttPosition.placement.split("-")[0]) || (tooltip.removeClass(lastPlacement.split("-")[0]), 
                                tooltip.addClass(ttPosition.placement.split("-")[0])), tooltip.hasClass(options.placementClassPrefix + ttPosition.placement) || (tooltip.removeClass(options.placementClassPrefix + lastPlacement), 
                                tooltip.addClass(options.placementClassPrefix + ttPosition.placement)), tooltip.hasClass("uib-position-measure") ? ($position.positionArrow(tooltip, ttPosition.placement), 
                        ...
                        scope.$on("$destroy", function() {
                            unregisterTriggers(), removeTooltip(), openedTooltips.remove(ttScope), ttScope = null;
                        });
                    };
                }
            };
    ...
    resetMatches(), scope.assignIsOpen = function(isOpen) {
        ...
        if (0 !== scope.matches.length && -1 !== HOT_KEYS.indexOf(evt.which)) {
            ...
            switch (evt.which) {
              ...
              case 38:
                ...
                scope.$digest(), target = popUpEl.find("li")[scope.activeIdx], target.parentNode.scrollTop = target.offsetTop;
                break;

              case 40:
                scope.activeIdx = (scope.activeIdx + 1) % scope.matches.length, scope.$digest(), 
                target = popUpEl.find("li")[scope.activeIdx], target.parentNode.scrollTop = target.offsetTop;
                break;
        ...
        })), !isEditable && modelCtrl.$error.editable && (modelCtrl.$setViewValue(), modelCtrl.$setValidity("editable", !0), 
        modelCtrl.$setValidity("parse", !0), element.val("")), hasFocus = !1, selected = !1;
    });
    ...
    showErrorsModule = angular.module("ui.bootstrap.showErrors", []), showErrorsModule.directive("showErrors", [ "$timeout", "showErrorsConfig", "$interpolate", function($timeout, showErrorsConfig, $interpolate) {
        var getShowSuccess, getTrigger, linkFn;
        return getTrigger = function(options) {
            var trigger;
            return trigger = showErrorsConfig.trigger, options && null != options.trigger && (trigger = options.trigger), 
            trigger;
        }, getShowSuccess = function(options) {
            var showSuccess;
            return showSuccess = showErrorsConfig.showSuccess, options && null != options.showSuccess && (showSuccess = options.showSuccess), 
            showSuccess;
        }, linkFn = function(scope, el, attrs, formCtrl) {
            var blurred, inputEl, inputName, inputNgEl, options, showSuccess, toggleClasses, trigger;
            if (blurred = !1, options = scope.$eval(attrs.showErrors), showSuccess = getShowSuccess(options), 
            trigger = getTrigger(options), inputEl = el[0].querySelector(".form-control[name]"), 
            inputNgEl = angular.element(inputEl), inputName = $interpolate(inputNgEl.attr("name") || "")(scope), 
            !inputName) throw "show-errors element has no child input elements with a 'name' attribute and a 'form-control' class";
            return inputNgEl.bind(trigger, function() {
            ...
            compile: function(elem, attrs) {
                if (-1 === attrs.showErrors.indexOf("skipFormGroupCheck") && !elem.hasClass("form-group") && !elem.hasClass("input-group")) throw "show-errors element does not have the 'form-group' or 'input-group' class";
                return linkFn;
            }
        };
    } ]), showErrorsModule.provider("showErrorsConfig", function() {
        var _showSuccess, _trigger;
        _showSuccess = !1, _trigger = "blur", this.showSuccess = function(showSuccess) {
    ...
    module.directive("vertilizeContainer", [ function() {
        return {
            ...
            controller: [ "$scope", "$window", function($scope, $window) {
                var _this = this;
                _this.childrenHeights = [], _this.allocateMe = function() {
                    return _this.childrenHeights.push(0), _this.childrenHeights.length - 1;
                }, _this.updateMyHeight = function(index, height) {
                    _this.childrenHeights[index] = height;
                }, _this.getTallestHeight = function() {
                    for (var height = 0, i = 0; i < _this.childrenHeights.length; i += 1) height = Math.max(height, _this.childrenHeights[i]);
                    return height;
        ...
        return {
            ...
            link: function(scope, element, attrs, parent) {
                var myIndex = parent.allocateMe(), getMyRealHeight = function() {
                    ...
                    return clone.remove(), realHeight;
                };
                scope.$watch(getMyRealHeight, function(myNewHeight) {
                    myNewHeight && parent.updateMyHeight(myIndex, myNewHeight);
                }), scope.$watch(parent.getTallestHeight, function(tallestHeight) {
                    tallestHeight && element.css("height", tallestHeight);
                });
            }
        };
    } ]);
